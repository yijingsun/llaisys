...
template <typename T>
void embedding_(T *out, const int64_t *index, const T *weight, size_t line_num, size_t vocab_size, size_t embedding_dim) {
...
void embedding(std::byte *out, const std::byte *index, const std::byte *weight, llaisysDataType_t type, size_t line_num, size_t vocab_size, size_t embedding_dim) {
    ...
    case LLAISYS_DTYPE_F32:
        return embedding_(reinterpret_cast<float *>(out), reinterpret_cast<const int64_t *>(index), reinterpret_cast<const float *>(weight), line_num, vocab_size, embedding_dim);
    case LLAISYS_DTYPE_BF16:
        return embedding_(reinterpret_cast<llaisys::bf16_t *>(out), reinterpret_cast<const int64_t *>(index),
                          reinterpret_cast<const llaisys::bf16_t *>(weight), line_num, vocab_size, embedding_dim);
    case LLAISYS_DTYPE_F16:
        return embedding_(reinterpret_cast<llaisys::fp16_t *>(out), reinterpret_cast<const int64_t *>(index),
                          reinterpret_cast<const llaisys::fp16_t *>(weight), line_num, vocab_size, embedding_dim);
    default:
