...
#include "moore/linear_moore.cuh"
#endif
...
void linear(tensor_t out, tensor_t in, tensor_t weight, tensor_t bias) {
    ...
    case LLAISYS_DEVICE_NVIDIA: {
        ...
        // nvidia::linear expects (M, N, K): in [M,K], weight [N,K], out [M,N]
        nvidia::linear(out->data(), in->data(), weight->data(), bias_data, out->dtype(),
        in->shape()[0], out->shape()[1], in->shape()[1], resource);
        return;
    }
#endif
#ifdef ENABLE_ILUVATAR_API
    case LLAISYS_DEVICE_ILUVATAR:
        return iluvatar::linear(out->data(), in->data(), weight->data(), bias_data, out->dtype(),
        in->shape()[0], in->shape()[1], out->shape()[1]);
#endif
#ifdef ENABLE_MOORE_API
    case LLAISYS_DEVICE_MOORE: {
        auto resource = llaisys::core::context().runtime().resource();
        // moore::linear expects (M, N, K): in [M,K], weight [N,K], out [M,N]
        moore::linear(out->data(), in->data(), weight->data(), bias_data, out->dtype(),
        in->shape()[0], out->shape()[1], in->shape()[1], resource);
        return;
    }
#endif
