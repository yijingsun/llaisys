...
#define CHECK_MUSA(call)                                                         \
    do {                                                                         \
        ...
        if (error != musaSuccess) {                                              \
            std::fprintf(stderr, "MUSA error: %s\n", musaGetErrorString(error)); \
            std::abort();                                                        \
        }                                                                        \
    } while (0)
...
void launch_argmax(int64_t *max_idx, T *max_val, const T *vals, size_t numel) {
    ...
    T *val_ptr = nullptr;
    CHECK_MUSA(musaMalloc(reinterpret_cast<void **>(&val_ptr), grid_size * sizeof(T)));
    int64_t *idx_ptr = nullptr;
    CHECK_MUSA(musaMalloc(reinterpret_cast<void **>(&idx_ptr), grid_size * sizeof(int64_t)));
    ...
    CHECK_MUSA(musaFree(val_ptr));
    CHECK_MUSA(musaFree(idx_ptr));
}
...
void argmax(std::byte *max_idx, std::byte *max_val, const std::byte *vals, llaisysDataType_t type, size_t numel) {
    ...
    case LLAISYS_DTYPE_BF16:
        return launch_argmax(
            reinterpret_cast<int64_t *>(max_idx),
            reinterpret_cast<__mt_bfloat16 *>(max_val),
            reinterpret_cast<const __mt_bfloat16 *>(vals),
            numel);
